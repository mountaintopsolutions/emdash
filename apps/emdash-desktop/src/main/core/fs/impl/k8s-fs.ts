 ...
 * cp / realpath), mirroring how SshFileSystem implements `search` and
 * `listRecursive` via exec. Binary reads and local→pod copies stream raw bytes
 * through the proxy's dedicated transfer helpers rather than the buffered,
 * UTF-8 `exec()` path which would corrupt non-text content.
 *
 * Watch is polling, mirroring SshFileSystem. All paths are POSIX.
 */
...
/**
 * K8sFileSystem implements FileSystemProvider over pod exec.
 ...
 */
export class K8sFileSystem implements FileSystemProvider {
  ...
  /**
   * List directory contents via `ls` + `stat` over exec.
   *
   * Uses a single `find -maxdepth 1` invocation that prints, per entry, a
   * record of type|size|mtime|mode|name so the whole listing comes back in one
   * round-trip (mirroring how SshFileSystem.listRecursive batches via find).
   */
   ...
   * path, then decodes as UTF-8, so the maxBytes truncation matches the SSH
   * implementation's byte semantics.
   */
  ...
  /**
   * Recursively list all files and directories via `find` (single round-trip).
   * Identical strategy to SshFileSystem.listRecursive.
   */
  async listRecursive(options?: { includeDirs?: boolean; maxEntries?: number }): Promise<{
    ...
    try {
      ...
      for (const line of effectiveLines) {
        ...
        items.push({ path: cleanRel, type: isDir ? 'dir' : 'file' });
      }
  ...
  async copyLocalFile(localAbsPath: string, destRelPath: string): Promise<void> {
    ...
    const data = await readFile(localAbsPath);
    await this.proxy.writeFileBytes(`cat > ${quoteShellArg(remoteFull)}`, data);
  }
  ...
  /**
   * Search for content in files via `grep` over exec. Identical parsing to
   * SshFileSystem.search.
   */
  ...
  /**
   * Read image file as base64 data URL via the proxy's binary transfer path.
   */
  async readImage(path: string): Promise<{
    ...
    if (size > maxImageSize) {
      return { success: false, error: `Image too large: ${size} bytes (max ${maxImageSize})` };
    }
    if (size === 0) {
