...
import type { ConnectionTestResult, K8sConfig, LocalPathEntry } from '@shared/kubernetes';
...
export function AddK8sConnModal({
  ...
  // If the kubeconfig defines exactly one context, select it automatically so
  // the user doesn't have to open a single-item dropdown. Only fills an empty
  // field, so an existing/edited selection is never overridden.
  const onlyContext = contexts.length === 1 ? contexts[0].name : undefined;
  useEffect(() => {
    if (onlyContext && !form.state.values.context) {
      form.setFieldValue('context', onlyContext);
                ...
                <form.Field name="kubeconfigPath">
                  {(field) => (
                    <Field>
                      ...
                      <KubeconfigPathPicker
                        id={field.name}
                        name={field.name}
                        value={field.state.value}
                        onBlur={field.handleBlur}
                        browsePath={(input) => k8sConnections.browsePath(input)}
                        onChange={(next) => {
                          field.handleChange(next);
                          setKubeconfigPath(next);
                      ...
                      <FieldDescription>
                        ...
                        ~/.kube/config). Start typing a path or `~` to browse.
                      </FieldDescription>
                    </Field>
                  )}
                </form.Field>
...
/**
 * Path input with a local file/folder browser dropdown. Folders are shown with
 * a trailing `/` and a folder icon; clicking one drills into it (the value gets
 * a trailing separator so the next listing is its contents), clicking a file
 * selects it. `~` is expanded by the backend.
 */
function KubeconfigPathPicker({
  id,
  name,
  value,
  onChange,
  ...
  onBlur: () => void;
  browsePath: (input: string) => Promise<{ entries: LocalPathEntry[] }>;
}) {
  const [open, setOpen] = useState(false);
  const [debounced, setDebounced] = useState(value);
  const inputRef = useRef<HTMLInputElement>(null);

  // Debounce so each keystroke doesn't fire a filesystem read.
  useEffect(() => {
    const timer = setTimeout(() => setDebounced(value), 120);
    return () => clearTimeout(timer);
  }, [value]);

  const browseQuery = useQuery({
    queryKey: ['k8sBrowsePath', debounced],
    queryFn: () => browsePath(debounced),
    enabled: open,
    retry: false,
  });
  const entries = browseQuery.data?.entries ?? [];

  const selectEntry = (entry: LocalPathEntry) => {
    if (entry.type === 'dir') {
      // Keep browsing inside the chosen folder.
      onChange(`${entry.path}/`);
  ...
  return (
    <div className="relative">
      ...
      {open && entries.length > 0 && (
        <div className="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto rounded-md bg-background-quaternary p-1 shadow-md ring-1 ring-foreground/10">
          {entries.map((entry) => (
            <button
              key={entry.path}
              type="button"
              // preventDefault so the input doesn't blur (closing the list)
              // before the click is handled.
              onMouseDown={(e) => e.preventDefault()}
              onClick={() => selectEntry(entry)}
              className="hover:text-accent-foreground flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-sm hover:bg-background-quaternary-1"
            >
              {entry.type === 'dir' ? (
